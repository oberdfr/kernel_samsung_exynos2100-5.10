...
static ssize_t vsync_show(struct class *class,
		struct class_attribute *attr, char *buf)
{
	struct dp_device *dp = get_dp_drvdata();

	return scnprintf(buf, PAGE_SIZE, "%llu\n",
			ktime_to_ns(dp->vsync.timestamp));
}
static CLASS_ATTR_RO(vsync);

static int dp_vsync_thread(void *data)
{
	struct dp_device *dp = data;

	while (!kthread_should_stop()) {
		ktime_t timestamp = dp->vsync.timestamp;
		int ret = wait_event_interruptible(dp->vsync.wait,
			(timestamp != dp->vsync.timestamp) &&
			dp->state == DP_STATE_ON);
		if (!ret)
			sysfs_notify(&dp->dev->kobj, NULL, "vsync");
	}

	return 0;
}

static int dp_create_vsync_thread(struct dp_device *dp, struct class *dp_class)
{
	int ret = 0;

	ret = class_create_file(dp_class, &class_attr_vsync);
	if (ret) {
		dp_err(dp, "failed to create dp vsync file\n");
		return ret;
	}

	init_waitqueue_head(&dp->vsync.wait);

	dp->vsync.thread = kthread_run(dp_vsync_thread, dp, "dp_vsync");
	if (IS_ERR_OR_NULL(dp->vsync.thread)) {
		dp_err(dp, "failed to run vsync thread\n");
		ret = PTR_ERR(dp->vsync.thread);
		dp->vsync.thread = NULL;
		goto err;
	}
...
err:
	class_remove_file(dp_class, &class_attr_vsync);
	return ret;
}
#endif
...
#ifdef FEATURE_ADD_VSYNC_NODE
			dp->vsync.timestamp = ktime_get();
			wake_up_interruptible_all(&dp->vsync.wait);
#endif
	}
...
#ifdef FEATURE_USE_DRM_EDID_PARSER
	dp_info(dp, "%s " DRM_MODE_FMT "\n", __func__, DRM_MODE_ARG(adjusted_mode));
	if (!drm_mode_match(&dp->cur_mode, adjusted_mode, DRM_MODE_MATCH_TIMINGS)) {
		dp_warn(dp, "mode mismatch, update cur_mode to new one\n");
...
#endif
#ifdef FEATURE_ADD_VSYNC_NODE
		dp_create_vsync_thread(dp, dp_class);
#endif
	}
