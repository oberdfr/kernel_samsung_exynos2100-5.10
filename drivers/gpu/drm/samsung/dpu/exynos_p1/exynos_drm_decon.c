		...
		if (atomic_inc_return(&exynos_crtc->d.shadow_timeout_cnt) >= TIMEOUT_CNT) {
			if (__is_recovery_supported(decon)) {
				if (!__is_recovery_running(decon))
					decon_trigger_recovery(exynos_crtc, RECOVERY_REQ_ALL);
				else
					goto exit;
			} else {
...
#if IS_ENABLED(CONFIG_DRM_MCD_COMMON)
	DPU_EVENT_LOG("MODE_SET", crtc, 0, "[MRES:%d, VREF:%d, mode:%s]",
			exynos_crtc_state->seamless_modeset & SEAMLESS_MODESET_MRES,
			exynos_crtc_state->seamless_modeset & SEAMLESS_MODESET_VREF,
...
{
	struct decon_device *decon =
		container_of(work, struct decon_device, off_work);
	struct drm_crtc *crtc = &decon->crtc->base;
