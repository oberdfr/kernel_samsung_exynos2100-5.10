		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	*/
	...
	if (xfer_sz > FCMD_DATA_MAX_SIZE) {
		dsim_err(dsim, "fcmd transfer size error(%lu, %d)\n", size, xfer_sz);
		return -1;
	}
	...
	dsim_info(dsim, "input size=%lu, xfer size=%d, xfer unit=%d, fcmd_buf_addr=0x%llx\n",
...
static bool dsim_check_clklane_stuck(u32 link, u32 dphy)
{
	...
	if (data == 0x1) {
		...
		pr_info(":recovery:matched=lp2hs\n");
	} else if (data == 0x05) {
		if (mask_value(dphy, DSIM_DPHY_STATUS_STOPSTATE_CLK,
				DSIM_DPHY_STATUS_STOPSTATE_CLK_SHIFT) != 0x0)
			goto exit;
		...
		pr_info(":recovery:matched=hs2lp(included datalane)\n");
	} else {
		goto exit;
	}
...
static bool dsim_check_datalane_stuck(u32 link, u32 dphy)
{
	...
	if (data == 0x02) {
		...
		if (mask_value(dphy, DSIM_DPHY_STATUS_TX_READY_HS_DATA,
			...
			goto exit;
		pr_info("recovery:%s:matched=lp2hs\n", __func__);
	} else if (data == 0x03) {
		if (mask_value(dphy, DSIM_DPHY_STATUS_TX_REQUEST_HS_DATA,
		...
		if (mask_value(dphy, DSIM_DPHY_STATUS_TX_READY_HS_DATA,
			...
			goto exit;
		pr_info("recovery:%s:matched=hs2lp\n", __func__);
	} else {
		goto exit;
	}
