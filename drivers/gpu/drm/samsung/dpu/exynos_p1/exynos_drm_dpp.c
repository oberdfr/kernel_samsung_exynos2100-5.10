				...
				const struct drm_display_mode *mode)
{
	...
	if (config->comp_type == COMP_TYPE_SBWCL) {
		...
		config->c_pl_stride = PL_STRIDE_SIZE_SBWC(config->src.f_w,
				is_10bpc);
	} else if (config->comp_type == COMP_TYPE_SAJC) {
		config->addr[1] = exynos_drm_fb_dma_addr(fb, 0);
	...
	if (!aclk_khz) {
		pr_warn("exynos_devfreq_get_domain_freq(DEVFREQ_DISP) returned 0!!!\n");
		config->rcv_num = 800000;
	} else
		config->rcv_num = aclk_khz;
#else
	aclk_khz = 800000;	/* max disp clock in khz */
	config->rcv_num = aclk_khz;
#endif
		...
		struct exynos_drm_plane_state *state)
{
	...
	if (!test_bit(DPP_ATTR_SAJC, &dpp->attr) &&
			(config.comp_type == COMP_TYPE_SAJC)) {
		dpp_err(dpp, "SAJC is not supported\n");
		goto err;
	}
...
static int dpp_remap_regs(struct dpp_device *dpp)
{
	...
	i = of_property_match_string(np, "reg-names", "votf_mscl");
	if (i >= 0) {
		res = platform_get_resource(pdev, IORESOURCE_MEM, i);
		ret = iommu_map(iommu_get_domain_for_dev(dpp->dev),
				res->start, res->start,
				res->end - res->start + 1, 0);
		if (ret) {
			dpp_err(dpp, "failed to iommu_map MSCL votf SFR region\n");
			goto err_dpp_debug;
		}

		ret = iommu_dma_reserve_iova(dpp->dev, res->start,
				res->end - res->start + 1);
		if (ret) {
			dpp_err(dpp, "failed to dma_reserve MSCL votf SFR region\n");
			goto err_dpp_debug;
		}
	}
