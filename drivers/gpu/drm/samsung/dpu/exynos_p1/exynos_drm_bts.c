...
void dpu_bts_set_bus_qos(const struct exynos_drm_crtc *exynos_crtc)
{
	...
	if ((decon->config.out_type & DECON_OUT_DSI)
		...
		&& (decon->bts.fps >= 63)) {
		pr_info("%s, acquire minlock (INT: 400, MIF: 546) width: %d, height: %d, fps: %d\n",
			__func__, decon->config.image_width, decon->config.image_height, decon->bts.fps);
...
static void dpu_bts_dp_qos_update(struct decon_device *decon)
{
	...
	DPU_DEBUG_BTS(decon, "pixelclock(%u)\n", pixelclock);
}
#endif
			...
			int max_overlay_cnt, u32 max_clk, u64 resol_clk, u32 *matrix, bool is_sajc)
{
	...
	if (is_sajc && dpu_bts_sajc_avg_ppc) {
		applied_ppc = ((max_tot_h * matrix[max_overlay_cnt])
				+ (non_max_tot_h * matrix[max_overlay_cnt - 1]))
...
dpu_bts_calc_sajc_disp_freq(struct decon_device *decon, u64 resol_clk, u32 max_clk)
{
	...
	get_private_modify_matrix(decon, &sajc_tdm_ppc[0]);
	...
	return need_win_overlay_clk(decon, &sajc_info, max_overlay_sajc_cnt,
				max_clk, resol_clk, &sajc_tdm_ppc[0], true);
}
...
static void dpu_bts_dsi_qos_update(struct decon_device *decon)
{
	...
	if ((decon->bts.max_disp_freq > decon->bts.dfs_lv[1]) &&
		(decon->bts.fps >= 120)) {
		if (exynos_pm_qos_read_req_value(PM_QOS_BUS_THROUGHPUT, &decon->bts.mif_qos) != mif_qos_min) {
			mif_qos_org = exynos_pm_qos_read_req_value(PM_QOS_BUS_THROUGHPUT, &decon->bts.mif_qos);
			exynos_pm_qos_update_request(&decon->bts.mif_qos, mif_qos_min);
		}
	} else if (exynos_pm_qos_read_req_value(PM_QOS_BUS_THROUGHPUT, &decon->bts.mif_qos) == mif_qos_min)
		exynos_pm_qos_update_request(&decon->bts.mif_qos, mif_qos_org);
}

static bool dpu_bts_disp_freq_up = true;
module_param_unsafe(dpu_bts_disp_freq_up, bool, 0600);
MODULE_PARM_DESC(dpu_bts_disp_freq_up, "whether to force up disp freq or not");
static void dpu_bts_find_max_disp_freq(struct decon_device *decon)
{
	...
	if (decon->config.out_type & DECON_OUT_DSI) {
		...
		if (dsim) {
			wclk = DIV_ROUND_UP(dsim->clk_param.hs_clk, 16);
	...
	if (dpu_bts_disp_freq_up) {
		if ((decon->bts.max_disp_freq > 200000) &&
			(decon->bts.max_disp_freq <= 332000)) {
			decon->bts.max_disp_freq = 400000;
			DPU_DEBUG_BTS(decon, "\tmax_disp_freq is raised from 332Mhz to 400MHz by force!\n");
		}
	}
		...
		struct bts_dpp_info *dpp, const struct dpu_bts_win_config *config)
{
	...
	DPU_DEBUG_BTS(decon,
			"\tDPP%d : bpp(%d) src w(%d) h(%d) rot(%d) comp(%d) hdr(%d) fmt(%s)\n",
			config->dpp_ch, dpp->bpp, dpp->src_w, dpp->src_h,
			dpp->rotation, dpp->compression, dpp->hdr, dpp->fmt_name);
	DPU_DEBUG_BTS(decon,
