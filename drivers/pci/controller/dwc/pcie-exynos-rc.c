...
static void exynos_gen3_nclkoff(void)
{
	static void __iomem *nclk_gen3;

	if (nclk_gen3 == NULL)
		nclk_gen3 = ioremap(0x11221064, 0x4);
	writel(0x5, nclk_gen3);
}
...
exit:
	dev_info(pci->dev, "not Force PCIe poweroff --> poweron\n");
...
EXPORT_SYMBOL(exynos_pcie_rc_dump_link_down_status);
...
int exynos_pcie_l1ss_ctrl(int enable, int id, int ch_num)
{
	...
	if (exynos_pcie->ep_device_type == EP_BCM_WIFI || exynos_pcie->ep_device_type == EP_QC_WIFI) {
		return	exynos_pcie_set_l1ss_wifi(enable, pp, id);
			...
			exynos_pcie->ep_device_type == EP_SAMSUNG_WIFI) {
		if (exynos_pcie->l1ss_enable)
			return	exynos_pcie_set_l1ss_samsung(enable, pp, id);
		dev_err(pci->dev, "Don't use PCIe L1ss on CH#%d(%d)\n", ch_num,
							exynos_pcie->l1ss_enable);
			return 0;
	}
	else
		return -EINVAL;
...
static int exynos_pcie_rc_make_reg_tb(struct device *dev, struct exynos_pcie *exynos_pcie)
{
	...
	while (pos) {
		...
		if (id >= PCI_MAX_CAP_NUM) {
			dev_err(dev, "Unexpected PCI_CAP number!\n");
			break;
		}
	...
	while (pos) {
		...
		if (id >= PCI_MAX_CAP_NUM) {
			dev_err(dev, "Unexpected PCI_EXT_CAP number!\n");
			break;
		}
...
static int exynos_pcie_runtime_resume(struct device *dev)
{
	...
	if ((exynos_pcie->ch_num == 0) && (exynos_pcie->chip_ver == 1))
		exynos_gen3_nclkoff();
